...
import ThanhVienModel from '../models/ThanhVienModel';
...
const nhomDangKyController = {
  ...
  dangKyNhom: async (nhomData) => {
    try {
      ...
      if (!nhomModel.thongTinNhomTruong.hoTen || !nhomModel.thongTinNhomTruong.maSinhVien) {
        throw new Error('Thông tin nhóm trưởng không đầy đủ');
      }

      // Kiểm tra danh sách thành viên
      const uniqueMaSV = new Set();
      uniqueMaSV.add(nhomModel.thongTinNhomTruong.maSinhVien);
      ...
      if (response && response.code === 1000) {
        ...
        throw new Error(response.message || 'Đăng ký nhóm thất bại');
      }
    } catch (error) {
